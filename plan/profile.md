# Profile and standing decisions

Source for everything below: plan/context/2026-09-08-claude-chat-transcripts.md (chat number in brackets). Extracted 2026-09-09.

## Who

- US citizen based in Denver, Colorado. Owns Superstoke, an LLC doing remote consulting for US clients (MarTech, Salesforce ecosystem, enterprise platform work). [1, 18]
- BA in art history, minor in business administration. 20+ years in marketing and technology. [9]
- Moving as a family of three: main applicant, spouse, one daughter of preschool or kindergarten age (exact age not recorded). Plus a dog. [1, 2, user 2026-09-09]
- Some Italian (not fluent). Learning Spanish now, Catalan after arrival. [10]

## Target

- Arrive in Spain May 2027. Academic start September or October 2027. [1]
- 100% committed to Spain. 80% Barcelona leaning, open to other Spanish cities if they make sense. Bologna and everything outside Spain are out. [4]
- Goal of the master: reposition out of marketing into corporate AI strategy, innovation, corporate AI governance and responsible AI, then pursue EU employment. Keep US-client consulting running throughout. [1]

## Programs, current state (2026-09-09)

- ELISAVA confirmed by email that it does not know whether the Master in Design for Responsible AI will run in 2027. It remains the primary target, and more Barcelona-based options are being added.
- ELISAVA's 2026 intake deadline was 2026-09-08. The 2027 deadline is assumed to be about 2027-09-08 until published.

## Income continuity (added 2026-09-09)

- The user will secure client contracts, at reduced rates if needed, that extend into May 2027 and beyond, so the visa file shows consistent income. Tracked in plan/workstreams/income-continuity.md.

## October 2026 trip and outreach (added 2026-09-09)

- Tentative Barcelona trip in October 2026: choose a neighborhood, visit universities, visit kindergartens for the daughter, check logistics, meet relocation agencies if needed, and meet Salesforce partners based in Barcelona. Tracked in plan/trip/ and plan/outreach/.

## Immigration and tax decisions

- Visa: Digital Nomad Visa (DNV). Main applicant on own LLC income. Spouse and child as dependents. Not a student visa. [1]
- Income threshold is proven by the main applicant alone. 2026 figures for a family of three: 2,849 + 1,069 + 357 = roughly 4,275 EUR per month gross. Dependents' income does not count. Keep a 10 to 20 percent USD to EUR buffer. [1, 5]
- Spouse's dependent authorization includes the right to work in Spain. [1]
- Filing route undecided: consular EX-01 (1-year visa, then 2-year renewals) or in-country EX-11 within 90 days of visa-free entry (3-year permit, about 20 business days). [1]
- Which consulate covers Colorado is unresolved: one chat says Los Angeles [2], another says Houston [3]. Verify before booking anything.
- Beckham Law (24 percent flat, Modelo 149 within 6 months of Social Security registration) is NOT assumed. Eligibility for an LLC owner depends on an administrator or director structure and needs a Spanish cross-border tax advisor before any planning relies on it. Fallback is standard autonomo IRPF plus RETA. [1]
- Consulting work done while physically in Spain is Spanish-source income and taxable there. [1]
- Long game: permanent residency after 5 years, citizenship after 10 (DELE A2 plus CCSE). [5]

## Program hard rules (apply to every program before it goes on the tracker)

1. No coding, ever. Any required Python, R, ML build work, or a coding bootcamp disqualifies the program. Tool-based, no-code analytics modules are acceptable but must be confirmed per syllabus. [1]
2. Not public policy, government, public affairs, regulation, or public governance. Target is corporate AI governance and strategy. [1]
3. Not a generic executive or MBA degree. The program must be AI-focused. Business schools are fine if the program itself is AI-focused. [1]
4. In Spain. [4]
5. 1 to 2 years, and compatible with running the consultancy (part-time, blended, or low-residency preferred). [1]
6. "AI in the degree name" is no longer a rule. Judge by curriculum weight. [1]
7. Program must be open to a 20-year senior professional, not built only for young graduates. Apply anyway where it is borderline (La Salle precedent). [1]
8. Titulo propio is acceptable for the DNV track. Prefer official where possible because the post-study job-search permit requires an official degree. [1]

## Standing instructions from the user to any assistant

- Answer only what is asked. No advice, motivation, or next steps unless asked. Short bullets. [1, 9]
- No em dashes anywhere.
- Do not relitigate whether to move to Europe or whether EU employment is realistic. Those are decided. [1]
